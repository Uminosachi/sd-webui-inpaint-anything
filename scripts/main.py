...
def download_model(sam_model_id):
    ...
    url_sam_vit_h_4b8939 = "https://dl.fbaipublicfiles.com/segment_anything/" + sam_model_id
...
def select_mask(masks_image):
    global sam_masks
    if sam_masks is None or masks_image is None:
        clear_cache()
        return None
...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as inpaint_anything_interface:
        with gr.Row():
                ...
                with gr.Row():
                    with gr.Column():
                        sam_model_id = gr.Dropdown(label="Segment Anything Model ID", elem_id="sam_model_id", choices=sam_model_ids,
                                                   value=sam_model_ids[0], show_label=False)
                    with gr.Column():
                        with gr.Column():
                            load_model_btn = gr.Button("Download model (if necessary)")
                        with gr.Column():
            ...
            sam_btn.click(run_sam, inputs=[input_image, sam_model_id], outputs=[sam_image])
